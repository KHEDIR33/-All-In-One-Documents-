...
const { pdfToWord } = require("../engines/pdfToWord/pdfToWord.engine");
const { createFileMetadata, markProcessed, deleteFileRecord } = require("../services/storage/fileRepository");
const { uploadTemporaryFile, removeTemporaryFile, BUCKET } = require("../services/storage/supabaseStorage");
const { getDeleteAt } = require("../services/filePolicy");

async function convertPdfToWord(req, res, next) {
  let outputDir;
  let fileId;
  let storagePath;
  ...
  try {
    ...
    storagePath = `processed/pdf-to-word/${fileId}/${crypto.randomUUID()}-${outputName}`;
